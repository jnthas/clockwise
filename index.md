...
<esp-web-install-button manifest="static/firmware/cw-cf-0x01/manifest.json">
  
  <button slot="activate">Mario Bros.</button>
...
<esp-web-install-button manifest="static/firmware/cw-cf-0x03/manifest.json">
  
  <button slot="activate">World Map</button>
  <span slot="unsupported">Ah snap, your browser does not support WebSerial API! If you are using a mobile browser, this is expected. Please, move to the desktop version.</span>
  <span slot="not-allowed">Ah snap, you are not allowed to use this on HTTP!</span>
</esp-web-install-button>
